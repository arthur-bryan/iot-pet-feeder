...
### A. Deploy frontend (Amplify)
- Amplify auto-deploys on push to `dev` branch
- Commit without author attribution, just describe changes
- `git add . && git commit -m "description" && git push origin dev`

### B. Deploy backend (Terraform/Lambda)
```bash
