...
async def subscribe_to_notifications(request: SubscribeRequest):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to subscribe: {str(e)}") from e
...
async def unsubscribe_from_notifications(request: UnsubscribeRequest):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to unsubscribe: {str(e)}") from e
