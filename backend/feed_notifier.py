...
def handler(event, context):
    ...
    - Pet ate (consumption events) - configurable
    - Feedings (status='completed') - configurable
    - Failures (status='failed') - always sent (cannot be disabled)
    ...
    try:
        ...
        # Get notification preferences (defaults if not set)
        preferences = email_config.get('preferences', {
            'pet_ate': False,
            'feedings': True,
            'failures': True
        })
        ...
        for record in event.get('Records', []):
            ...
            # Check if this is a consumption event (pet ate)
            if event_type == 'consumption':
                if not preferences.get('pet_ate', False):
                    print(f"Skipping consumption event (pet_ate notifications disabled): {feed_id}")
                    continue

                # Send consumption notification
                subject = "Pet Feeder: Pet Ate"
                weight_before = new_image.get('weight_before', {}).get('N', 'N/A')
                weight_after = new_image.get('weight_after', {}).get('N', 'N/A')
                weight_delta = new_image.get('weight_delta', {}).get('N', 'N/A')

                message = f"""Your pet ate from the feeder!

Feed ID: {feed_id}
Time: {timestamp}
Weight before: {weight_before}g
Weight after: {weight_after}g
Amount consumed: {weight_delta}g
...
"""
            ...
            if status == 'completed':
                # Check if feeding notifications are enabled
                if not preferences.get('feedings', True):
                    print(f"Skipping completed feed (feeding notifications disabled): {feed_id}")
                    continue
...
"""
                ...
                # Failures are ALWAYS sent (cannot be disabled)
