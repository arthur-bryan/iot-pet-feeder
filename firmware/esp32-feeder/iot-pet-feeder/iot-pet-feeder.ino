...
// Configuration
const char* API_BASE_URL = "https://fk40h8b7gf.execute-api.us-east-2.amazonaws.com/dev/api/v1/config";
...
float getWeight() {
    if (!scaleInitialized) {
        Serial.println("getWeight() called but scale NOT initialized!");
        return -1.0f;
    }
    ...
    if (weight == -1.0f) {
        Serial.println("HX711 read failed, attempting power cycle...");
        scale.power_down();
        delay(200);
        scale.power_up();
        delay(200);
        ...
        if (weight == -1.0f) {
            Serial.println("HX711 still failed after power cycle");
            return -1.0f;
        }
    }

    Serial.printf("Raw weight reading: %.2fg (before clamping)\n", weight);

    float finalWeight = max(0.0f, weight);
    if (weight < 0.0f) {
        Serial.printf("WARNING: Negative weight %.2fg clamped to 0.0g\n", weight);
    }

    lastValidWeight = finalWeight;
    Serial.printf("Final weight: %.2fg\n", finalWeight);
    return finalWeight;
}
...
void setup() {
    ...
    Serial.printf("  DOUT Pin: %d, SCK Pin: %d\n", LOADCELL_DOUT_PIN, LOADCELL_SCK_PIN);
    ...
    if (retries >= 20) {
        Serial.println("\n*** HX711 INIT FAILED - Scale not responding ***");
        scaleInitialized = false;
    } else {
        Serial.println("\nHX711 ready - Scale is responding");
        Serial.printf("  Setting calibration factor: %.2f\n", CALIBRATION_FACTOR);
        ...
        delay(500);
        float testReading = scale.get_units(5);
        Serial.printf("  Post-tare test reading: %.2fg\n", testReading);
...
void publishFeedEvent(const char* trigger, const char* status) {
    ...
    if (strcmp(trigger, "button") == 0) {
        ...
        doc["mode"] = "api";
        doc["requested_by"] = activeFeedRequestedBy.length() > 0 ? activeFeedRequestedBy.c_str() : "api_user";
        doc["event_type"] = "manual_feed";
    } else if (strcmp(trigger, "scheduled") == 0) {
        doc["mode"] = "scheduled";
        doc["requested_by"] = activeFeedRequestedBy.length() > 0 ? activeFeedRequestedBy.c_str() : "scheduler";
